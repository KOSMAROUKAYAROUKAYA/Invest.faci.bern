...
def depot():
  ...
  if request.method == 'POST':
    ...
    nouveau_depot = Depot(
        montant=montant, user_id=user.id, operateur=operateur, id_transaction=id_trans
    )
    db.session.add(nouveau_depot)
    db.session.commit()
    message = (
        'Demande de dépôt soumise avec succès. En attente de validation.'
    )
...
def investir():
  ...
  return render_template(
      'investir.html', user=user, niveaux=niveaux, message=message
  )
...
def retrait():
  ...
  if request.method == 'POST':
    ...
    if user.niveau_actuel == 0:
      message = (
          'Erreur : Vous devez investir avant de pouvoir effectuer un retrait.'
      )
    elif not (2500 <= montant_brut <= 50000):
      message = (
          'Erreur : Le montant de retrait doit être compris entre 2500F et'
          ' 50000F.'
      )
    else:
