...
def tableau_bord():
if 'user_id' not in session: return redirect(url_for('login_user'))
...
retraits_ce_mois = Retrait.query.filter(Retrait.user_id == user.id, Retrait.date_creation >= debut_mois).count()

if retraits_ce_mois >= 3:
message = "Erreur : Vous avez atteint la limite de 3 retraits par mois."
else:
montant_net = montant_brut * 0.8
nouveau_retrait = Retrait(montant=montant_net, montant_brut=montant_brut, user_id=user.id)
...
@app.route('/admin')
def admin_panel():
if session.get('telephone') != 'kosmaRoukaya': return redirect(url_for('login_user'))

depots = db.session.query(Depot, Utilisateur).join(Utilisateur, Depot.user_id == Utilisateur.id).filter(Depot.statut == 'En attente').all()
retraits = db.session.query(Retrait, Utilisateur).join(Utilisateur, Retrait.user_id == Utilisateur.id).filter(Retrait.statut == 'En attente').all()
investisseurs = Utilisateur.query.all()

return render_template('admin.html', depots=depots, retraits=retraits, investisseurs=investisseurs)
